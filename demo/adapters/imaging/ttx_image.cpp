...
static void bind_operation(
    const Godot::Demo::Imaging::Graph::Operation& definition,
  ...
  godot::gdextension_interface::classdb_register_extension_class_method(
      godot::gdextension_interface::library, class_name._native_ptr(), &method);
}

void Godot::Demo::Adapters::Imaging::TtxImage::_bind_methods() {
  ...
  for (const auto* operation :
       Godot::Demo::Imaging::Graph::Vocabulary::standard().get_operations()) {
    bind_operation(*operation, call_operation);
  }
}

Godot::Demo::Adapters::Imaging::TtxImage::~TtxImage() {
  if (expression) {
    expression->release();
  }
...
void Godot::Demo::Adapters::Imaging::TtxImage::set_provider(
    const godot::String& value) {
  if (Godot::Demo::Imaging::Graph::Observation::active()) {
...
void Godot::Demo::Adapters::Imaging::TtxImage::set_provider_object(
...
auto Godot::Demo::Adapters::Imaging::TtxImage::get_provider_object() const
    -> godot::Ref<godot::RefCounted> {
  return provider_object;
}

auto Godot::Demo::Adapters::Imaging::TtxImage::get_provider() const
    -> godot::String {
...
auto Godot::Demo::Adapters::Imaging::TtxImage::get_error() const
    -> godot::String {
  return error;
}

auto Godot::Demo::Adapters::Imaging::TtxImage::open_provider(
  ...
  const auto accept = [&](image_provider factory) {
    supply = &Godot::Demo::Imaging::Graph::Provider::adopt(
        factory, Godot::Demo::Imaging::Graph::Vocabulary::standard());
    return true;
  };
...
auto Godot::Demo::Adapters::Imaging::TtxImage::set_rgba8(
  ...
  const auto invalid =
      Godot::Demo::Imaging::Contracts::Pixels::validate(w, h, bytes);
  if (!invalid.is_empty()) {
  ...
  auto result =
      Godot::Demo::Imaging::Graph::Image::create(*supply, w, h, bytes, errors);
  return result.visit(
      [&](Godot::Demo::Imaging::Graph::Image& image) {
        if (source) {
          ...
          source = &Godot::Demo::Imaging::Graph::Source::create(
              Core::Data::take(image));
          expression = source;
        }
...
auto Godot::Demo::Adapters::Imaging::TtxImage::evaluate(
    Memory::Allocator::Arena& errors)
  ...
  return result.visit(
      [](const Godot::Demo::Imaging::Graph::Image& image) { return &image; },
      [&](Core::View::Bytes message)
          -> const Godot::Demo::Imaging::Graph::Image* {
        error = host_string(message);
        return nullptr;
...
auto Godot::Demo::Adapters::Imaging::TtxImage::get_provider_statistics()
    -> godot::Dictionary {
  Godot::Demo::Imaging::Graph::Observation observation;
  Memory::Allocator::Arena errors;
...
auto Godot::Demo::Adapters::Imaging::TtxImage::get_width() -> int64_t {
  Memory::Allocator::Arena errors;
  const auto image = evaluate(errors);
  return image ? image->get_width() : 0;
}

auto Godot::Demo::Adapters::Imaging::TtxImage::get_height() -> int64_t {
  Memory::Allocator::Arena errors;
  const auto image = evaluate(errors);
...
// dependencies.
auto Godot::Demo::Adapters::Imaging::TtxImage::snapshot()
    -> godot::Ref<TtxImage> {
  Memory::Allocator::Arena errors;
  const auto image = evaluate(errors);
  if (!image) {
  ...
  output.instantiate();
  output->source = &Godot::Demo::Imaging::Graph::Source::create(*image);
...
auto Godot::Demo::Adapters::Imaging::TtxImage::read_pixels()
...
auto Godot::Demo::Adapters::Imaging::TtxImage::apply(
    const Godot::Demo::Imaging::Graph::Operation& operation,
  ...
  Memory::Dynamic::Vector<Godot::Demo::Imaging::Graph::Call::Argument>
      arguments;
  GDExtensionInt index = 0;
  if (operation.get_input() ==
      Godot::Demo::Imaging::Graph::Operation::Input::Image) {
    if (index >= count) {
    ...
    const auto& kernel =
        constants->construct<Godot::Demo::Imaging::Graph::Kernel>(
            w, h,
  ...
  auto& call = Godot::Demo::Imaging::Graph::Call::create(
  ...
  const bool success = result.visit(
      [](const Godot::Demo::Imaging::Graph::Image&) { return true; },
      [&](Core::View::Bytes message) {
        error = host_string(message);
        return false;
      });
...
void Godot::Demo::Adapters::Imaging::TtxImage::call_operation(
    void* definition,
  ...
  const auto output = resource.apply(
      *static_cast<const Godot::Demo::Imaging::Graph::Operation*>(definition),
      arguments, count);
...
auto Godot::Demo::Adapters::Imaging::TtxImage::get_offers() -> godot::Array {
  Godot::Demo::Imaging::Graph::Observation observation;
  Memory::Allocator::Arena errors;
  godot::Array result;
  const auto* image = evaluate(errors);
  ...
  image->get_query().bind<Godot::Demo::Imaging::Contracts::Offers>().visit(
      [&](Godot::Demo::Imaging::Contracts::Offers offers) {
      ...
      [&](Ttx::Semantic::Negotiation::Binding::Failure failure) {
        error = host_string(
            Godot::Demo::Imaging::Graph::Image::binding_error(failure));
      });
  return result;
}

auto Godot::Demo::Adapters::Imaging::TtxImage::admit(
...
auto Godot::Demo::Adapters::Imaging::TtxImage::invoke(
    const godot::String& name,
  ...
  const Godot::Demo::Imaging::Graph::Operation* operation = nullptr;
  image->get_query().bind<Godot::Demo::Imaging::Contracts::Offers>().visit(
      [&](Godot::Demo::Imaging::Contracts::Offers offers) {
        offers.visit([&](image_offer offer) {
          if ((Core::View::Bytes(offer.name.data, offer.name.size) == label ||
            ...
            operation = &constants->construct<
                Godot::Demo::Imaging::Graph::Operation>(
                Godot::Demo::Imaging::Graph::Operation::described(
                    label, System::Uuid(offer.contract),
                    static_cast<Godot::Demo::Imaging::Graph::Operation::Input>(
                        offer.input)));
          }
        });
      },
