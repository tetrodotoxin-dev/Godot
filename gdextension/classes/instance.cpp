...
auto Gdextension::Classes::Instance::create(
    Class& type,
  ...
  Core::Option<::Gdextension::Contracts::Lifecycle> lifecycle;
  if (type.is_node()) {
    auto status =
        publication.get_query()
            .bind<::Gdextension::Contracts::Lifecycle>()
            .visit(
                [&](::Gdextension::Contracts::Lifecycle value) {
                  lifecycle = value;
...
auto Gdextension::Classes::Instance::notify(S32 notification) -> void {
  if (!lifecycle) {
    return;
  }
