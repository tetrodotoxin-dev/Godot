...
static auto expose(
  ...
  return policy.get_interface().bind<::Gdextension::Contracts::GDClass>().visit(
      [&](::Gdextension::Contracts::GDClass gdclass) -> Result {
        return gdclass.emit().visit(
            [](Ttx::Semantic::Ownership::Publication& terminal) -> Result {
              return Core::Data::take(terminal);
            },
            [&](Ttx::Data::Status) -> Result { return policy.get_error(); });
      },
...
auto Gdextension::Modules::Classes::compile(const godot::Dictionary& config)
    -> Utility::
        Result<Ttx::Semantic::Ownership::Publication, Core::View::Bytes> {
  using Result =
      Utility::Result<Ttx::Semantic::Ownership::Publication, Core::View::Bytes>;
...
auto Gdextension::Modules::Classes::load(const godot::Array& configuration)
    -> void {
  for (int64_t index = 0; index < configuration.size(); ++index) {
    ...
    compile(entry).visit(
        [&](Ttx::Semantic::Ownership::Publication& terminal) {
          ...
          auto status =
              terminal.get_query()
                  .bind<::Gdextension::Contracts::Class>()
                  .visit(
                      [](::Gdextension::Contracts::Class type) {
                        return type.publish();
                      },
...
Gdextension::Modules::Classes::~Classes() {
