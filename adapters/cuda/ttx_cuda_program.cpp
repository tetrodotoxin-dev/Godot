...
auto Godot::Adapters::Cuda::TtxCudaProgram::_bind_methods() -> void {
...
auto Godot::Adapters::Cuda::TtxCudaProgram::compile_file(
...
auto Godot::Adapters::Cuda::TtxCudaProgram::compile(
    const godot::String& source,
    const godot::Dictionary& headers,
    const godot::PackedStringArray& options) -> bool {
  error = "";
  if (!module) {
    Gdextension::Modules::Imports imports(
    ...
    imports
        ...
        .visit(
            ...
            [&](Ttx::Data::Status) {
              error = "The project has no available CUDA compiler import.";
            });
    if (!module) {
      return false;
    }
...
auto Godot::Adapters::Cuda::TtxCudaProgram::allocate(int64_t size)
...
auto Godot::Adapters::Cuda::TtxCudaProgram::prepare(
