...
counter_statistics counter_metrics;

static counter_statistics snapshot(const void* source) {
  (void)source;
  return counter_metrics;
}

const counter_inspection_operations counter_inspection = {snapshot};
...
static S64 live(const void* source) {
  (void)source;
  return (S64)(counter_metrics.instances_opened -
               counter_metrics.instances_closed);
}
...
static ttx_data_status create(const void* source, ttx_publication* output) {
  ...
  ++counter_metrics.instances_opened;
...
static ttx_binding_status
  ...
  if (id.high == COUNTER_INSPECTION_HIGH && id.low == COUNTER_INSPECTION_LOW) {
    const counter_inspection_api api = {source, &counter_inspection};
...
static void release_factory(const void* source) {
  (void)source;
  ++counter_metrics.factories_closed;
}

ttx_data_status counter_emit_factory(ttx_publication* output) {
  ++counter_metrics.factories_opened;
