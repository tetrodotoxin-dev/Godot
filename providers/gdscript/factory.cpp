...
Providers::Gdscript::Factory::Factory(
    U8* allocation,
...
auto Providers::Gdscript::Factory::create(
    const godot::Ref<godot::RefCounted>& object)
  ...
  static const image_provider_operations operations = {
    ...
    [](const void* source, U32 width, U32 height, const U8* data, Count size,
      ...
      const auto invalid =
          Contracts::Pixels::validate(width, height, {data, size});
      if (!invalid.is_empty()) {
        return {invalid.get_data(), invalid.get_size()};
      }

      // The C input is borrowed for this call. The script may retain its
      // argument, so it receives a Godot owned array with an independent life.
...
auto Providers::Gdscript::Factory::open(
