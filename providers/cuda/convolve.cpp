...
static auto transform(
    const Providers::Cuda::Image& image,
    Contracts::Kernel::Coefficients kernel)
...
auto Providers::Cuda::Convolve::apply(const Image& image, image_kernel kernel)
  ...
  auto output = transform(
      image,
      Contracts::Kernel::Coefficients{kernel.width, kernel.height, weights});
  return output.visit(
