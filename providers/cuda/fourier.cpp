...
#include "providers/cuda/fourier.hpp"
...
Providers::Cuda::Fourier::~Fourier() {
  reset();
}

void Providers::Cuda::Fourier::reset() {
  if (!context) {
    return;
  }
...
auto Providers::Cuda::Fourier::prepare(CUcontext supplied, U32 w, U32 h)
    -> Core::View::Bytes {
...
auto Providers::Cuda::Fourier::forward() -> Core::View::Bytes {
  CurrentContext current(context);
  if (current.status != CUDA_SUCCESS) {
    return "cuFFT context is unavailable."_view;
  }
...
auto Providers::Cuda::Fourier::inverse() -> Core::View::Bytes {
  CurrentContext current(context);
  if (current.status != CUDA_SUCCESS) {
    return "cuFFT context is unavailable."_view;
  }
