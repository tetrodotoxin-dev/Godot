...
#include "imaging/cuda/composite.hpp"
...
static auto blend(const Imaging::Cuda::Image& image, Core::View::Bytes pixels)
  ...
  return uploaded.visit(
      [&](CUdeviceptr overlay)
          -> Utility::Result<Imaging::Cuda::Allocation, Core::View::Bytes> {
        const Imaging::Cuda::Allocation foreground(runtime, overlay);
        auto allocated = runtime.allocate(pixels.get_size());
        return allocated.visit(
            [&](CUdeviceptr output)
                -> Utility::Result<
                    Imaging::Cuda::Allocation, Core::View::Bytes> {
...
auto Imaging::Cuda::Composite::apply(const Image& image, image_object overlay)
  ...
  return Imaging::Contracts::Pixels::read(overlay).visit(
