...
auto Imaging::Graph::Call::create(
    Expression& receiver,
    const Operation& operation,
    Core::View::Vector<Argument> arguments,
...
auto Imaging::Graph::Call::cached(Memory::Allocator::Arena& errors)
...
auto Imaging::Graph::Call::evaluate_value(
    Memory::Allocator::Arena& errors,
    U64 pull) -> Utility::Result<const Image&, Core::View::Bytes> {
  if (last_pull == pull) {
  ...
  if (failed.is_empty()) {
    if (!binding || receiver_changed) {
      ...
      target->bind_operation(operation).visit(
          [&](const Invocation& value) { binding = value; },
...
auto Imaging::Graph::Call::resolve_concept(Core::View::Bytes name) const
    -> Ttx::Concept::Abstract {
  if (name == "operation"_view) {
    return Ttx::Concept::Abstract::provide(operation);
  }

  if (name == "receiver"_view) {
    return Ttx::Concept::Abstract::provide(receiver);
  }

  if (name == "value"_view && output) {
    return Ttx::Concept::Abstract::provide(*output);
  }
  ...
  for (Count i = 0; i < inputs.get_size(); ++i) {
    ...
    if (name == Core::View::Bytes(index)) {
      ...
      return argument.expression
                 ? Ttx::Concept::Abstract::provide(*argument.expression)
                 : Ttx::Concept::Abstract::provide(*argument.constant);
    }
  }
...
void Imaging::Graph::Call::visit_concepts(
    Ttx::Concept::Abstract::Visitor visitor) const {
  visitor("operation"_view, Ttx::Concept::Abstract::provide(operation));
  visitor("receiver"_view, Ttx::Concept::Abstract::provide(receiver));
  if (output) {
    visitor("value"_view, Ttx::Concept::Abstract::provide(*output));
  }
